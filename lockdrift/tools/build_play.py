...
DESCRIPTION = (
    "Which of your dependencies would change if you installed right now? Your "
    "manifest declares a range, your lockfile pins a version, and your installed "
    "tree holds a third answer -- and they disagree more often than anyone "
    "checks. This reads all three off your disk and reports each dependency as "
    "DRIFT (the sources disagree in a way your next install acts on), UNLOCKED "
    "(declared, but nothing pins it), UNCHECKED (a source could not be read or "
    "observed) or SYNCED (all three read, all three agree). A source that was not "
    "read is never reported as agreement: no node_modules means the installed "
    "state was not observed, so those rows are UNCHECKED and never SYNCED. Range "
    "evaluation is deliberately narrow -- carets, tildes, exact pins and single "
    "comparators are evaluated, while unions, hyphen ranges and git, file or "
    "workspace references are reported unevaluated rather than guessed. It never "
    "installs, never resolves against a registry, and never runs a package "
    "manager, so it cannot tell you the newest version -- only whether what you "
...
GRAPH = [
    # The three readers touch no shared state and depend on nothing but root,
    # so they are parallel root steps and the join fans in. Two layers.
    ("read_declared", "read_declared", 60_000, ["root"], []),
    ("read_locked", "read_locked", 60_000, ["root"], []),
    ("read_installed", "read_installed", 90_000, ["root"], []),
    ("compare_pins", "compare_pins", 15_000,
     ["--from-steps", "edge:read_declared", "edge:read_locked", "edge:read_installed"],
     ["read_declared", "read_locked", "read_installed"]),
]
...
def argv_for(script, spec, parents):
    ...
    for token in spec:
        if token == "root":
            ...
            argv.append(EDGE.format(step=token[len("edge:"):]))
        else:
            argv.append(token)
    return argv
